...
class IneligiblePagesSpec extends ItSpec with TableDrivenPropertyChecks {
  ...
  val listOfIneligibleReasons: TableFor5[String, Reason, String, BasePage, String] = Table(
    ...
    ("direct debit(s) created within the last 24 months", DirectDebitAlreadyCreated, "already have a plan", alreadyHaveAPlanPage, "4900.00")
  )
  ...
  def beginJourney(ineligibleReason: Reason): Unit = {
    ...
    getBanksIsSuccessful(if (ineligibleReason == DirectDebitAlreadyCreated) almostAYearAgo else aYearAgo)
