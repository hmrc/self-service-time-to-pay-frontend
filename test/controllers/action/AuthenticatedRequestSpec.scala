  ...
  "hasActiveSaEnrolment" should {
    "return true" when {
      "the SA enrolment exists and is activated" in {
        new AuthenticatedRequest(request, Enrolments(Set(saEnrolment)), None, None).hasActiveSaEnrolment shouldBe true
      }
    }
    ...
    "return false" when {
      "the SA enrolment exists and is not activated" in {
        new AuthenticatedRequest(
          request, Enrolments(Set(unactivatedSaEnrolment)), None, None).hasActiveSaEnrolment shouldBe false
      }

      "the SA enrolment does not exist" in {
        new AuthenticatedRequest(request, Enrolments(Set.empty), None, None).hasActiveSaEnrolment shouldBe false
      }
    }
  }
}
