...
class ValidateBankDetailsResponseSpec extends FreeSpec with Matchers with ScalaCheckPropertyChecks {
  ...
  "isInvalid" - {
    "should return the correct value" in {
      forAll(testCases) { case testData @ (expectedIsValid: Boolean, _: Option[String], _: Option[String], _: Option[String], _: String) =>
    ...
    "test cases are exhaustive and cover all combinations" in {
      ...
      val testDataResponses = testCases.map{ case testData @ (_: Boolean, _: Option[String], _: Option[String], _: Option[String], _: String) =>
