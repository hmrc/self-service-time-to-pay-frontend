...
class CalculatorServiceMaximumDurationInMonthsSpec extends PlaySpec with TableDrivenPropertyChecks {
  ...
  "maximumDurationInMonths for" should {
    TableDrivenPropertyChecks.forAll(scenariosBasedAroundJanuaryDuedate) { (startDate: LocalDate, returnsAndDebits: ReturnsAndDebits, answer: Int) =>
      s"scenarios Based Around January Due date return the correct number of months($answer) for start date : $startDate and SelfAssessmentDetails $returnsAndDebits" in {
    ...
    TableDrivenPropertyChecks.forAll(scenariosBasedAroundJulyDuedate) { (startDate: LocalDate, returnsAndDebits: ReturnsAndDebits, answer: Int) =>
      s"scenarios Based Around July Due date return the correct number of months($answer) for start date : $startDate and SelfAssessmentDetails $returnsAndDebits" in {
        val result: Int = maximumDurationInMonths(returnsAndDebits, startDate)
        assert(result == answer)
    ...
    TableDrivenPropertyChecks.forAll(scenariosBasedAroundEarlyFilers) { (startDate: LocalDate, returnsAndDebits: ReturnsAndDebits, answer: Int) =>
      s"scenarios Based Around Early Filers Due date return the correct number of months($answer) for start date : $startDate and SelfAssessmentDetails $returnsAndDebits" in {
        val result: Int = maximumDurationInMonths(returnsAndDebits, startDate)
        assert(result == answer)
      }
    }
    ...
    TableDrivenPropertyChecks.forAll(scenariosBasedAroundEarlyFilers) { (startDate: LocalDate, returnsAndDebits: ReturnsAndDebits, answer: Int) =>
      s"scenarios Based Around Late Filers Due date return the correct number of months($answer) for start date : $startDate and SelfAssessmentDetails $returnsAndDebits" in {
        val result: Int = maximumDurationInMonths(returnsAndDebits, startDate)
        assert(result == answer)
      }
    }

    TableDrivenPropertyChecks.forAll(scenariosBasedCustom) { (startDate: LocalDate, selfA: ReturnsAndDebits, answer: Int) =>
