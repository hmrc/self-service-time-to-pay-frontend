...
import com.github.tomakehurst.wiremock.client.WireMock.{aResponse, equalToJson, post, postRequestedFor, stubFor, urlPathEqualTo, verify}
import com.github.tomakehurst.wiremock.stubbing.StubMapping
import org.scalatest.concurrent.Eventually
import org.scalatest.time.{Seconds, Span}
import play.api.libs.json.JsObject

object AuditStub extends Eventually {
  ...
  def verifyEventAudited(auditType: String, auditEvent: JsObject): Unit = eventually {
    verify(
      ...
      postRequestedFor(urlPathEqualTo(auditUrl))
        ...
        .withRequestBody(
          equalToJson(s"""{ "auditSource" : "pay-what-you-owe"  }""", true, true)
        )
        .withRequestBody(
