...
import ssttpcalculator.model.{TaxPaymentPlan, Debit, TaxLiability, Instalment, PaymentSchedule}
...
class CalculatorService @Inject() (
  ...
  def availablePaymentSchedules(sa: SelfAssessmentDetails, initialPayment: BigDecimal = BigDecimal(0),
    ...
    rangeOfAvailableScheduleDurationsInMonths.map { durationInMonths =>
      ...
      val calculatorInput: TaxPaymentPlan = CalculatorService.changeCalculatorInput(
        durationInMonths,
        dayOfMonth,
        initialPayment,
        debits
  ...
  def buildSchedule(implicit calculatorInput: TaxPaymentPlan): PaymentSchedule = {
    ...
    val initialPaymentInterest: BigDecimal = if (hasAnInitialPayment) {
      ...
      val debitsDueBeforeInitialPayment: Seq[TaxLiability] = calculatorInput.liabilities.filter(_.dueDate.isBefore(initialPaymentDate))
  ...
  def createInstalments(remainingLiabilities: Seq[TaxLiability], monthyRepayment: Any, repaymentDates: Seq[LocalDate]) = ???
  ...
  def createInstalments(liabilities: Seq[TaxLiability], monthlyRepayment: BigDecimal, repaymentDates: Seq[LocalDate]): Seq[Instalment] = {
    val result = repaymentDates.foldLeft((liabilities, Seq.empty[Instalment])){
      case ((ls, rl), dt) => (ls, rl :+ Instalment(dt, monthlyRepayment, 0))
    }
    result._2
  }
...
object CalculatorService {
  ...
  def makeCalculatorInput(debits: Seq[TaxLiability], initialPayment: BigDecimal, durationInMonths: Int)
