...
class ArrangementController @Inject() (
  ...
  def getInstalmentSummary: Action[AnyContent] = as.authorisedSaUser.async { implicit request =>
    ...
    journeyService.authorizedForSsttp { journey =>
      ...
      val schedule: PaymentSchedule = calculatorService.computeSchedule(journey).getOrElse(
        throw new RuntimeException("could not calculate a valid schedule but there should be one")
      )
        ...
        dayOfMonthForm.bindFromRequest().fold(
          ...
          (validFormData: ArrangementForm) => {
            ...
            journeyService.saveJourney(updatedJourney).map {
              _ => Redirect(ssttpcalculator.routes.CalculatorController.getPaymentPlanOptions())
            }
          }
        )
    }
  ...
  def submit(): Action[AnyContent] = as.authorisedSaUser.async { implicit request =>
    tryLock(request.request.session.get(SessionKeys.sessionId).getOrElse("unknown-session")) {
      ...
      journeyService.authorizedForSsttp { ttp =>
        ...
        val paymentSchedule: PaymentSchedule = calculatorService.computeSchedule(ttp).getOrElse (
          throw new RuntimeException("could not calculate a valid schedule but there should be one")
        )
