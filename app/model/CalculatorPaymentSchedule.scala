...
import uk.gov.hmrc.selfservicetimetopay.models.CalculatorPaymentScheduleInstalment
...
final case class CalculatorPaymentSchedule(startDate:            Option[LocalDate],
  ...
  def initialPaymentScheduleDate: LocalDate = instalments.map(_.paymentDate).minBy(_.toEpochDay)
}

object CalculatorPaymentSchedule {
  implicit val format: Format[CalculatorPaymentSchedule] = Json.format[CalculatorPaymentSchedule]
}
