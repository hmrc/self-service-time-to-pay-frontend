...
import uk.gov.hmrc.selfservicetimetopay.service.CalculatorService.getMonthRange
...
class CalculatorService @Inject()(calculatorConnector: CalculatorConnector,
  ...
  def getInstalmentsSchedule(sa: SelfAssessment)(implicit hc: HeaderCarrier, ec: ExecutionContext): Future[Map[Int, CalculatorPaymentSchedule]] = {

    val input: List[(Int, CalculatorInput)] = getMonthRange(sa).map(month => (month,validateCalculatorDates(CalculatorDef.apply(month), month, sa.debits))).toList
